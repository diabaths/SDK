...
namespace D_Ezreal_SDK_
{
    internal static class DamageIndicator
    {
        internal class DamageInfo
        {
            ...
            internal readonly Func<SharpDX.Color> Color;
        ...
        internal static void Initialize(List<DamageInfo> damageInfoList, Func<bool> enabled, Func<bool> herosEnabled)
        {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            targets.Where(h => h.IsValid() && h.IsHPBarRendered).ForEach(unit =>
            {
                ...
                _damageInfoList.Where(x => x.Enabled()).OrderByDescending(x => x.DamageCalcFunc(unit)).ForEach(damageinfo =>
                {
                    ...
                    Drawing.DrawLine(drawStartXPos, barPos.Y, drawEndXPos, barPos.Y, height, Color.FromArgb(170, damageinfo.Color().R, damageinfo.Color().G, damageinfo.Color().B));
